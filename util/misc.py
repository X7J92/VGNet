...
class TensorDict(object):
    def __init__(self, tensor_dict,extra_data=None):
...
def tensor_dict_from_dict_list(tensor_list: List[dict]):
    ...
    for key in tensor_list[0].keys():
        if isinstance(tensor_list[0][key], torch.Tensor):
            tensor_dict[key] = torch.stack([t[key] for t in tensor_list], dim=0)
        else:
            # For data that are not tensors, we store it in the extra_data dictionary.
            extra_data[key] = [t[key] for t in tensor_list]
    return TensorDict(tensor_dict, extra_data=extra_data)
