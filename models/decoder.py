...
from .ff import MultiheadAttention
# from . import MultiheadAttention1
...
class vg_decoder_wrapper(nn.Module):
    ...
    def forward(self, img_feat, mask, pos_embed, word_feat, word_mask,stage_attention_masks):
        bs = stage_attention_masks.shape[0]

        final_list = []

        for i in range(bs):
            chunks = torch.split(stage_attention_masks[i], split_size_or_sections=1, dim=0)
            chunks = [chunk.squeeze(0) for chunk in chunks]
            # 这个 mask 检查每行是否全为0
            masks = [torch.any(chunk != 0) for chunk in chunks]
            # 过滤出非零行
            non_zero_chunks = [chunk for chunk, mask in zip(chunks, masks) if mask]
            # 将每个 batch 的过滤结果添加到 final_list
            final_list.append(non_zero_chunks)

        final_list1=[]
        for item in final_list:
          item= torch.stack(item)
          b_m = item == 0
          final_list1.append(b_m)
        word_mask = final_list1
...
class MultiStageDecoderLayer(nn.Module):
    ...
    def forward(self, vis_query, vis_query_pos, text_query_pos,
        ...
        # Iterative  Cross-modal Decoder


        bs = word_feat.shape[1]
        text_info_list = []
        for i in range(bs):
            word_feat1 = word_feat[:, i, :].unsqueeze(1)
            vis_query1 = vis_query[:, i, :].unsqueeze(1)
            vis_query_pos1 = vis_query_pos[:, i, :].unsqueeze(1)
            tensor = word_key_padding_mask[i]
            numbers = tensor.shape[0]

            for j in range(numbers):
                current_tensor = tensor[j].unsqueeze(0)
                vis_query1 = self.with_pos_embed(vis_query1, vis_query_pos1)
                text_info = self.word_attn(query=vis_query1,
                                       key=self.with_pos_embed(word_feat1, word_pos),
                                       value=word_feat1, key_padding_mask=current_tensor)[0]
                vis_query1 = text_info
            text_info_list.append(text_info)
        text_info = torch.cat(text_info_list, dim=1)
...
class DiscriminativeFeatEncLayer(nn.Module):
    def __init__(self, d_model, img2text_attn_args=None, img_query_with_pos=True,
        ...
        self.mha = MultiheadAttention1(embed_dim=256, num_heads=8)  # 不需要再传入参数，因为它们已经在类中被固定
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
        ...
        last_rows = [tensor[-1, :] for tensor in word_key_padding_mask]
        ...
        # visual-linguistic verification粗粒度匹配
        img_query = img_feat + img_pos if self.img_query_with_pos else img_feat
        text_info1 = self.img2text_attn(
            query=img_query, key=self.with_pos_embed(word_feat, word_pos),
            value=word_feat, key_padding_mask=last_rows)[0]
        # text_info = self.img2img_attn(
        #     query=text_info1, key=img_query, value=img_query, key_padding_mask=img_key_padding_mask)[0]
        text_embed = self.text_proj(text_info1)
        ...
        verify_score = self.tf_scale * \
                       torch.exp( - (1 - verify_score).pow(self.tf_pow) \
                        / (2 * self.tf_sigma**2))
        img_feat = img_feat*verify_score
        ...
        bs=img_feat.shape[1]
        for i in range(bs):
            img_feat1 = img_feat[:,i,:].unsqueeze(1)
            word_feat1 = word_feat[:,i,:].unsqueeze(1)
            img_key_padding_mask1=img_key_padding_mask[i,:].unsqueeze(1).permute(1,0)
            tensor = word_key_padding_mask[i]
            numbers = tensor.shape[0]
            attn_probs1 =None
            for j in range(numbers):
                current_tensor = tensor[j].unsqueeze(0)
                text_cond_info, attn_probs0= self.mha(
                    query=img_feat1, key=self.with_pos_embed(word_feat1, word_pos),
                    value=word_feat1, key_padding_mask=current_tensor,attn_probs1=attn_probs1)
                attn_probs1 =attn_probs0
                q = k = img_feat1 + text_cond_info
                text_cond_img_ctx = self.img2img_attn(
                    query=q, key=k, value=img_feat1, key_padding_mask=img_key_padding_mask1)[0]
                img_feat1 = text_cond_img_ctx
            text_cond_info_list.append(text_cond_img_ctx)
        text_cond_info_list = torch.cat(text_cond_info_list,dim=1)
        fuse_img_feat = (self.norm_img(orig_img_feat) +
                         self.norm_text_cond_img(text_cond_info_list)) * verify_score
...
class MultiheadAttention1(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                ...
                key_padding_mask: torch.Tensor = None,attn_probs1 =None) -> torch.Tensor:
        # Assuming q, k, v are of shape (seq_len, batch_size, embed_dim)
        ...
        if key_padding_mask is not None:
            # Adjusting the mask for multi-heads
            expanded_mask = key_padding_mask.unsqueeze(1).repeat(1, self.num_heads, 1)
            expanded_mask = expanded_mask.reshape(bsz * self.num_heads, src_len)
            scores.masked_fill_(expanded_mask.unsqueeze(1).expand(-1, tgt_len, -1), float('-inf'))

        attn_probs0 = F.softmax(scores, dim=-1)
        attn_probs0 = self.dropout(attn_probs0)

        if attn_probs1 is None:
            attn_probs0 = attn_probs0  # 这行代码实际上没有必要，因为值并没有改变
        else:
            attn_probs0 = attn_probs0 - attn_probs1/10

        attn_output = torch.matmul(attn_probs0, v)

        # Reshape attn_output to (seq_len, bsz, embed_dim)
        attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)

        attn_output = self.out_proj(attn_output)

        return attn_output, attn_probs0
